#[derive(Debug)]
struct Rectangle {
    width: u32,
    height: u32,
}

impl Rectangle { // в этой области всё связывается с типом
    fn area(&self) -> u32 {
        self.height * self.height
    }

    fn width(&self) -> bool {
        self.width > 0
    }

    fn can_hold(&self, other: &Rectangle) -> bool {
        self.width > other.width && self.height > other.height
    }fn square(size: u32) -> Self {
        Self {
            width: size,
            height: size,
        }
    }


}

fn main() {
    let width1: u32 = 30;
    let height1: u32 = 50;

    println!(
        "The area of the rectangle is {} square pixels.",
        area(width1, height1)
    );
    
    let rectangle1: (u32, u32) = (25, 50);
    println!(
        "The area of the rectangle1 is {} square pixels.",
        area_tuple(rectangle1)
    );
    
    let rectangle2 = Rectangle {
        width: 35,
        height: 50,
    };
    println!(
        "The area of the rectangle2 is {} square pixels.",
        area_struct(&rectangle2)
    );
    
    println!("Rectangle2 is: {:#?}", rectangle2);
    
    let scale = 2;
    
    let rectangle3 = Rectangle {
        width: dbg!(25 * scale),
        height: 50,
    };
    
    println!(
        "The area of the rectangle3 is {} square pixels.",
        rectangle3.area()
    );
    if rectangle3.width() {
        println!("The rectangle3 has a nonzero width; it is {}", rectangle3.width);
    }
    
    dbg!(rectangle3);
    
    let rect1 = Rectangle {
        width: 30,
        height: 50,
    };
    let rect2 = Rectangle {
        width: 10,
        height: 40,
    };
    let rect3 = Rectangle {
        width: 60,
        height: 45,
    };
    
    println!("Can rect1 hold rect2? {}", rect1.can_hold(&rect2));
    println!("Can rect1 hold rect3? {}", rect1.can_hold(&rect3));
    
    let square = Rectangle::square(3);
    
    println!(
        "The area of the square is {} square pixels.",
        square.area()
    );
    if square.width() {
        println!("The square has a nonzero width; it is {}", square.width);
    }
    
}

fn area(width: u32, heigth: u32) -> u32 {
    width * heigth
}

fn area_tuple(dimension: (u32, u32)) -> u32 {
    dimension.0 * dimension.1
}

fn area_struct(rectangle: &Rectangle) -> u32 {
    rectangle.width * rectangle.height
}
